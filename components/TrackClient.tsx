...
import { Search, ArrowLeft, ArrowRight, Receipt, Loader2, ShieldCheck, CheckCircle2, XCircle, Clock, Star, MessageSquare, Terminal } from "lucide-react";
...
export default function TrackClient() {
  ...
  const handleTrack = async (e: React.FormEvent) => {
    ...
    try {
      // Menggunakan logika aslimu (api/system?type=orders)
  ...
  return (
    <div className="flex flex-col min-h-screen bg-darker selection:bg-brand-blue/30 relative overflow-hidden w-full">
      ...
      >
        ...
        <div className="hidden md:flex gap-8 text-sm font-medium text-gray-300">
          ...
          <Link href="/track" className="text-brand-cyan transition-colors font-bold flex items-center border-b border-brand-cyan pb-1">
      ...
      {/* 🎯 KONTEN TENGAH (TRACKING AREA) */}
      <main className="relative z-10 flex-grow flex flex-col items-center justify-center pt-40 pb-20 px-4 w-full max-w-4xl mx-auto">
            ...
            {!orderData ? (
              ...
              <motion.div key="search" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full">
                <div className="text-center mb-8 md:mb-10">
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-xs font-bold text-gray-300 mb-6 backdrop-blur-md mx-auto">
                    <ShieldCheck className="w-4 h-4 text-green-400" /> Secure Order Tracking
                  </div>
                  <h1 className="text-4xl md:text-6xl font-extrabold text-white mb-3 md:mb-4 tracking-tight">
                    Lacak <span className="text-gradient">Pesanan</span>
                  </h1>
                  <p className="text-gray-400 text-sm md:text-lg max-w-md mx-auto">Masukkan Nomor Invoice (Order ID) Anda untuk melihat status pembayaran dan mengambil lisensi.</p>
                </div>
                    ...
                    <button 
                      ...
                      className={`w-full py-4 md:py-5 rounded-2xl font-bold text-base md:text-lg flex items-center justify-center transition-all duration-300 shadow-lg ${
                        isProcessing || !invoiceId
                          ...
                          : "bg-gradient-to-r from-brand-blue to-brand-cyan hover:opacity-90 text-white hover:-translate-y-1 shadow-[0_0_20px_-5px_rgba(37,99,235,0.4)]"
                      }`}
                    >
                      {isProcessing ? (
                ...
                <div className="glass p-5 md:p-10 rounded-[2rem] border border-white/10 shadow-2xl relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 md:mb-8 border-b border-white/10 pb-5 md:pb-6">
                      <div>
                        ...
                        {orderData.status === 'Sukses' ? (
                          <div className="flex items-center text-xl md:text-2xl font-black text-green-400"><CheckCircle2 className="w-5 h-5 md:w-6 md:h-6 mr-2" /> LUNAS</div>
                    ...
                    {orderData.status === 'Sukses' && orderData.licenseKey && (
                      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-gradient-to-r from-brand-blue/10 to-brand-cyan/5 border border-brand-blue/30 rounded-2xl p-5 md:p-6 mb-6 md:mb-8 relative overflow-hidden group">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-brand-blue/20 blur-[40px] rounded-full pointer-events-none" />
                        <div className="relative z-10">
                          <p className="text-brand-cyan text-[10px] md:text-xs font-bold uppercase tracking-widest mb-3 flex items-center"><ShieldCheck className="w-3 h-3 md:w-4 md:h-4 mr-1"/> License Key Resmi Anda</p>
                          
                          <div className="flex flex-col sm:flex-row gap-3 items-center">
                            <code className="flex-1 w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 font-mono text-base md:text-lg text-white font-bold tracking-widest text-center">
                              {orderData.licenseKey}
                            </code>
                            <button 
                              onClick={() => copyLicense(orderData.licenseKey)}
                              className="w-full sm:w-auto bg-brand-blue hover:bg-blue-500 text-white px-6 py-3 rounded-xl font-bold transition-colors shadow-lg"
                            >
                              Salin Lisensi
                            </button>
                          </div>
      ...
      {/* 🏁 FOOTER PROFESIONAL */}
      <footer className="border-t border-white/10 pt-20 pb-8 px-4 relative z-10 bg-darker/90 backdrop-blur-xl mt-auto w-full">
